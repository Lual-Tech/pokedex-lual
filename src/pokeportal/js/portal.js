...
/* Prefixo "pp:" = poke-portal. Evita conflito com outras páginas do mesmo domínio.
   Cada chave guarda UMA parte do estado da aplicação (tudo como string, pq localStorage só aceita string). */
const AUTH_KEY       = 'pp:auth';        // "true"/"false" → usuário logado ou não
const VIEW_STATE_KEY = 'pp:view';        // "login" | "register" | "profile" | "pokemon"
const FAVORITES_KEY  = 'pp:favorites';   // '["0007","0008"]' → lista de IDs favoritos
const MIGRATION_OLD  = 'pokemonFavorites'; // chave antiga (legada) usada em versões anteriores

/*************** DOM ROOT ***************/
/* appContainer é o “root” onde eu injeto TUDO (SPA simples sem framework). */
const appContainer = document.getElementById('app');

/*************** DADOS MOCK ***************/
/* Simulo usuário e um Pokémon para a demo. Em produção, isso viria da API. */
...
const squirtleData = {
  ...
  evolutions:[
    // Imagens do repositório público da PokeAPI (SVG estiloso do “dream-world”)
...
/*************** FAVORITOS: MIGRAÇÃO E CRUD ***************/
/* MIGRAÇÃO da chave antiga → nova:
   - Se existir “pokemonFavorites” (versão antiga), uno com “pp:favorites” atual, removo duplicados e salvo em FAVORITES_KEY.
   - Depois deleto a chave antiga para não acumular lixo.
   - try/catch: se o JSON antigo estiver quebrado, a app não explode. */
(function migrateOldFavorites(){
  const old = localStorage.getItem(MIGRATION_OLD);
  if (!old) return;
  try{
    const cur = JSON.parse(localStorage.getItem(FAVORITES_KEY) || '[]'); // favoritos atuais (novo formato)
    const legacy = JSON.parse(old) || [];                                 // favoritos antigos
    const merged = Array.from(new Set([...(cur||[]), ...(legacy||[])]));   // remove duplicados via Set
    localStorage.setItem(FAVORITES_KEY, JSON.stringify(merged));           // persiste só na chave nova
    localStorage.removeItem(MIGRATION_OLD);                                // limpa a chave antiga
  }catch{}
})();

/* Leitura segura: se o JSON estiver inválido ou vazio, retorna [] e segue o jogo. */
const getFavorites = () => {
  try { return JSON.parse(localStorage.getItem(FAVORITES_KEY) || '[]'); }
  catch { return []; }
};
/* Checagem simples: “esse ID está na lista de favoritos?” */
const isFavorite = (id) => getFavorites().includes(id);
/* Toggle favorito:
   - Lê os favoritos, vira Set (mais fácil de adicionar/remover sem duplicados).
   - Se já era favorito → remove; senão → adiciona.
   - Salva a lista nova (transformando o Set em array com spread).
   - Mostra um toast (alertMessage) com feedback.
   - Re-renderiza views que exibem estado de favorito (pokemon/profile). */
const toggleFavorite = (id) => {
  ...
  if (getCurrentView()==='pokemon') renderPokemonPage(); // atualiza ícone do coração
  if (getCurrentView()==='profile') renderProfilePage(); // atualiza lista de favoritos
};

/*************** AUTH “FAKE” (DEMO) ***************/
/* isLoggedIn: lê AUTH_KEY e compara com 'true' (string). */
const isLoggedIn = () => localStorage.getItem(AUTH_KEY) === 'true';
/* loginUser: marca 'true', mostra toast e redireciona para o “app” (rota de exemplo). */
const loginUser   = () => { localStorage.setItem(AUTH_KEY,'true');  alertMessage('Login bem-sucedido!'); window.location.href = "/src/index.html"; };
/* logoutUser: marca 'false' e manda para a view 'login'. */
const logoutUser  = () => { localStorage.setItem(AUTH_KEY,'false'); alertMessage('Logout realizado.');   setView('login'); };

/*************** HELPERS UI ***************/
/* createBadge: devolve um span estilizado (badge). Se for tipo gradiente, forço “inline-flex” para centralizar. */
...
/* SVGs soltos para reutilizar como ícone no HTML gerado (facilita legibilidade do template). */
const BackIcon = `
...
/* alertMessage: “toast” minimalista.
   - Cria um #alert-box se não existir.
   - Mostra mensagem e some sozinho depois de 3s (com fade). */
...
/* Persisto a última view no localStorage:
   - getCurrentView(): lê “pp:view” (ou “login” por padrão).
   - setView(v): salva e força re-render (SPA simples). */
...
function renderLoginPage(){
  ...
  /* HTML com comentários inline explicando cada bloco visual.
     OBS: comentários dentro do template devem ser HTML <!-- ... -->. */
  appContainer.innerHTML = `
  <!-- main: cartão central do formulário de login -->
  <main class="w-full max-w-lg md:max-w-xl mx-auto p-8 rounded-xl shadow-2xl bg-card-dark border border-gray-700">
    <!-- section: cabeçalho com botões “SSO” (Google/Apple) -->
    <section class="text-center mb-8" aria-label="Área de Login">
      <h1 class="text-2xl font-bold mb-6">Faça login para continuar</h1>
      <div class="space-y-3">
        <!-- Botão “Continuar com Google” (apenas visual na demo) -->
        ...
        <!-- Botão “Continuar com Apple” (apenas visual na demo) -->
        <button type="button" class="w-full flex items-center justify-center p-3 rounded-lg font-semibold border border-gray-600 bg-gray-700 hover:bg-gray-600 transition"
          style="background-color:var(--cinzaesc);border-color:var(--cinzaclaro);">
      ...
      <!-- divisor “OU” entre SSO e formulário clássico -->
    ...
    <!-- form: login com usuário/senha (na demo chama loginUser direto ao submeter) -->
    <form id="login-form" aria-label="Formulário de Login com Credenciais">
      ...
      <!-- Campo de senha + link de esqueci minha senha (não implementado na demo) -->
      <div class="mb-4">
  ...
  // JS que dá vida ao formulário e aos botões “visuais”
...
function renderRegisterPage(){
  ...
  appContainer.innerHTML = `
  <!-- main: cartão de cadastro com campos básicos -->
  <main class="w-full max-w-lg md:max-w-xl mx-auto p-8 rounded-xl shadow-2xl bg-card-dark border" style="border-color:var(--cinzaesc);">
    ...
    <!-- form: validação simples (senhas iguais) e volta para login na sequência -->
    <form id="register-form" aria-label="Formulário de Cadastro">
      <!-- Nome -->
      <div class="mb-4">
        <label class="block text-sm font-medium mb-1">Nome de usuário</label>
        <input type="text" id="name" required class="w-full p-3 rounded-lg input-dark" placeholder="Nome">
      </div>
      <!-- E-mail -->
      <div class="mb-4">
        <label class="block text-sm font-medium mb-1">E-mail</label>
        <input type="email" id="email" required class="w-full p-3 rounded-lg input-dark" placeholder="usuario@gmail.com">
      </div>
      <!-- Telefone -->
      <div class="mb-4">
        <label class="block text-sm font-medium mb-1">Telefone</label>
        <input type="tel" id="phone" required class="w-full p-3 rounded-lg input-dark" placeholder="(00) 0000-0000">
      </div>
      <!-- Senha -->
      <div class="mb-4">
        <label class="block text-sm font-medium mb-1">Escolha sua senha</label>
        <input type="password" id="reg-password" required class="w-full p-3 rounded-lg input-dark" placeholder="******">
      </div>
      <!-- Confirmar senha -->
      <div class="mb-4">
        <label class="block text-sm font-medium mb-1">Confirme sua senha</label>
        <input type="password" id="confirm-password" required class="w-full p-3 rounded-lg input-dark" placeholder="******">
      </div>
  ...
  // Validação simplona: só confere se as senhas batem
...
function renderProfilePage(){
  ...
  // Lê favoritos persistidos e decide o conteúdo da seção “Pokémon Favoritos”
  const favorites = getFavorites();
  const isSquirtleFavorite = favorites.includes(squirtleData.id);
  const favoritesContent = isSquirtleFavorite
    ? `<!-- Botão que leva ao Pokémon favoritado -->
       <button class="w-full p-3 rounded-lg flex items-center justify-between transition cursor-pointer"
         ...
         <div class="flex items-center">
           <img src="${squirtleData.imageUrl}" alt="${squirtleData.name}" class="w-8 h-8 object-contain mr-3">
           <span class="font-medium">${squirtleData.name}</span>
         </div>
         ${HeartIcon(true)}
       </button>`
  ...
  appContainer.innerHTML = `
  <!-- Cartão do perfil: capa + avatar + seções -->
  <div class="w-full max-w-2xl md:max-w-4xl mx-auto rounded-xl shadow-2xl bg-card-dark border" style="border-color:var(--cinzaesc);">
    ...
    <section class="px-8 pb-8 space-y-6">
      ...
      <!-- Ações rápidas + seção de favoritos -->
      <div class="space-y-3">
        <!-- Link que navega para a página de Pokémon (detalhes) -->
        ...
        <!-- Conteúdo da lista de favoritos (ou estado vazio) -->
  ...
  // Listeners dos elementos da página de perfil
...
/* ============================
   TABS DO POKÉMON (Sobre/Status/Evoluções)
   ============================ */
/* Aba SOBRE: descrição + badges + atributos básicos em grid. */
function renderPokemonAboutTab(p){
  ...
  return `
    ...
    <!-- Listas de Fraquezas e Resistências com badges coloridas -->
...
/* Aba STATUS: barras de progresso com valores base. */
function renderPokemonStatusTab(p){
  const line = (stat)=>{
    const pct = (stat.value/stat.max)*100;
    return `
      <!-- Linha de status: nome + valor + barra percentuada -->
      <div class="flex items-center space-x-4 py-2 border-b last:border-b-0" style="border-color:var(--cinzaesc);">
  ...
  return `
    <div class="w-full max-w-lg mx-auto pt-4">
      ${p.stats.map(line).join('')}
      <p class="text-xs text-center mt-4" style="color:var(--cinzaclaro);">
        Valores de Status Base. (Máximo: ${p.stats[0].max})
      </p>
    </div>`;
}
/* Aba EVOLUÇÕES: lista em ordem com setas e nível de evolução. */
function renderPokemonEvolutionsTab(p){
  const evos = p.evolutions.map((evo,i,arr)=>{
    const isCurrent = evo.id===p.id;                               // true se for a forma atual
    const border = isCurrent ? 'var(--primary-secondary)' : 'transparent';
    const block = `
      <!-- Bloco de uma forma evolutiva -->
    ...
    if (i < arr.length-1){
      const next = arr[i+1];
      return block + `
        <!-- Conector visual com nível necessário para a próxima evolução -->
  ...
  return `
    <div class="w-full max-w-sm mx-auto pt-4 space-y-4">
      <h3 class="text-xl font-bold text-center mb-6">Cadeia Evolutiva</h3>
      ${evos}
    </div>`;
}
...
function renderPokemonPage(){
  ...
  const favorited = isFavorite(p.id); // lê do localStorage para pintar o coração preenchido/vazio
  ...
  <div class="w-full max-w-2xl md:max-w-4xl mx-auto rounded-xl shadow-lg pokemon-card border overflow-hidden" style="border-color:var(--cinzaesc);">
    <!-- Cabeçalho com gradiente do tipo + título + ação de favoritar -->
    <header class="relative p-6 pokemon-header min-h-[250px] flex flex-col justify-between">
      <div class="flex justify-between items-center z-10">
        ...
        <!-- Título e botão de favorito (coração) -->
        <div class="flex items-center space-x-4">
      ...
      <!-- Ilustração centralizada (decorativa) -->
    ...
    <!-- Conteúdo em abas: Sobre | Status | Evoluções -->
    <div class="p-8 space-y-8 rounded-b-xl" style="background-color:var(--card-dark);">
      ...
      <!-- Área onde o conteúdo da aba é injetado -->
  ...
  // Lógica das abas: troca classes visuais e injeta o HTML correspondente
  ...
  switchTab('about'); // começa na aba “Sobre”

  // Ações: voltar para perfil, abrir perfil pela bottom-nav e alternar favorito (com persistência)
...
/*************** APP SHELL ***************/
/* renderApp decide qual “tela” mostrar a partir de dois estados PERSISTIDOS:
   1) AUTH_KEY → se o usuário está logado.
   2) VIEW_STATE_KEY → última tela visitada.
   Regras:
     - Logado e em login/register? vou para profile (pós-login).
     - Deslogado e view ≠ register? mando para login.
     - Caso contrário, renderizo a view pedida. */
function renderApp(){
